    ...
    <form action="{{url('admin/config')}}" method="post">
        ...
        <table class="add_tab">
            ...
            <tr>
                <th>类型：</th>
                <td>
                    <input type="radio" class="sm" name="field_type" onclick="showTR()" value="input" @if(old('field_type')=='input') checked @endif>input　
                    <input type="radio" class="sm" name="field_type" onclick="showTR()" value="textarea" @if(old('field_type')=='textarea') checked @endif>textarea　
                    <input type="radio" class="sm" name="field_type" onclick="showTR()" value="radio" @if(old('field_type')=='radio') checked @endif>radio
                </td>
            </tr>
            <tr class="TR">
                <th>类型值：</th>
            ...
            <tr>
                <th>内容：</th>
                <td>
                    <textarea name="conf_content" id="" cols="30" rows="10">{{old('conf_content')}}</textarea>
                </td>
            </tr>
            <tr>
                <th>排序：</th>
                <td>
                    <input type="text" class="sm" name="conf_order" value="{{old('conf_order')??'0'}}">
                </td>
            </tr>
            <tr>
                <th>说明：</th>
                <td>
                    <textarea name="conf_tips" id="" cols="30" rows="10">{{old('conf_tips')}}</textarea>
                </td>
            </tr>
            <tr>
                <th></th>
